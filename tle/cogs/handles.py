...
class Handles(commands.Cog):
    ...
    async def _set(self, ctx, member, user):
        ...
        if not roles:
            try:
                color = discord.Colour(user.rank.color_embed) if user.rank.color_embed is not None else discord.Colour.default()
                role_to_assign = await ctx.guild.create_role(name=user.rank.title, color=color, reason="TLE bot required role")
                logging.info(f'Created missing role {user.rank.title} in guild {ctx.guild.name}')
            except discord.Forbidden:
                raise HandleCogError(f'Role for rank `{user.rank.title}` not present in the server and I lack permissions to create it.')
            except discord.HTTPException as e:
                raise HandleCogError(f'Failed to create role for rank `{user.rank.title}`: {e}')
        else:
    ...
    async def _update_ranks(self, guild, res):
        ...
        if missing_roles:
            for role_name in missing_roles:
                rank = next((r for r in cf.RATED_RANKS + (cf.UNRATED_RANK,) if r.title == role_name), None)
                color = discord.Colour(rank.color_embed) if rank and rank.color_embed is not None else discord.Colour.default()
                try:
                    role = await guild.create_role(name=role_name, color=color, reason="TLE bot required role")
                    rank2role[role_name] = role
                    logging.info(f'Created missing role {role_name} in guild {guild.name}')
                except discord.Forbidden:
                    raise HandleCogError(f'Role for rank `{role_name}` not present in the server and I lack permissions to create it.')
                except discord.HTTPException as e:
                    raise HandleCogError(f'Failed to create role for rank `{role_name}`: {e}')
    ...
    async def _generic_remind(self, ctx, action, role_name, what):
        ...
        if not roles:
            try:
                role = await ctx.guild.create_role(name=role_name, reason="TLE bot required role")
                logging.info(f'Created missing role {role_name} in guild {ctx.guild.name}')
            except discord.Forbidden:
                raise HandleCogError(f'Role `{role_name}` not present in the server and I lack permissions to create it.')
            except discord.HTTPException as e:
                raise HandleCogError(f'Failed to create role `{role_name}`: {e}')
        else:
            role = roles[0]
        if action == 'give':
